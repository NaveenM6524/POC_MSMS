...
int main(void)
{
    ...
    const int MAX_SESSION_LOGIN_ATTEMPTS = 5;
    ...
    while (running) {
        ...
        while (!loggedIn) {
            ...
            if (!loggedIn) {
                ...
                if (sessionAttempts >= MAX_SESSION_LOGIN_ATTEMPTS) {
                    printf("Too many failed login attempts. Exiting.\n");
                    logEvent("SYSTEM", "SESSION_LOCKOUT",
                             "max login attempts reached, program exiting");
                    running = 0;
                    break;
                }
